...
class GenericProductExcelService
{
    private const HEADER_ROW = 3; // русские заголовки (читаем/пишем)
    private const LABEL_ROW  = 4; // тех. ключи (пишем, импорт не использует)
    private const DATA_ROW   = 5; // с какой строки начинаются данные
    ...
    public function preview(string $path, string $sheetName, int $templateId, int $limit = 10): array
    {
        ...
        $headerRow = $this->trimArray($headerRow);
        ...
        $labelRow = $this->trimArray($labelRow);
    ...
    private function trimArray(array $arr): array
    {
